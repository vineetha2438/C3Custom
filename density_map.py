...
def save_computed_density(density_map, out_path, density_path):
    """
    Save density map to h5py format
    """
    with h5py.File(out_path, 'w') as hf:
        hf['density'] = density_map
    with open(density_path, 'w'):
        np.savetxt(density_path, density_map, delimiter=",")
...
def get_gt_dots(mat_path, img_height, img_width):
    ...
    ** cliping is needed to prevent going out of the array
    ...
    gt = mat["annPoints"].astype(np.float32).round().astype(int)
    if(bool(gt.shape[0])):
        gt[:,0] = (gt[:,0].clip(0, img_width - 1))*2
        gt[:,1] = (gt[:,1].clip(0, img_height - 1))*2
    return gt
...
def compute_distances(out_dist_path='distances_dict.pkl', Shanghai_root_path='./', n_neighbors = 4, leafsize=1024):
    ...
    for full_img_path in tqdm(full_img_pathes):
        mat_path = full_img_path.replace('.png','.mat').replace('images','ground_truth')

        img = plt.imread(full_img_path)
        non_zero_points = get_gt_dots(mat_path, *img.shape[0:2])

        if(bool(non_zero_points.shape[0])):
            ...
            distances = np.zeros((non_zero_points.shape[0], 4))
...
compute_distances(out_dist_path=precomputed_distances_path, Shanghai_root_path='D:\CNN\C-3-Framework\datasets\\NTU\data\\train_data')
...
data_root  = 'D:\CNN\C-3-Framework\datasets\\NTU\data\\train_data'
img_pathes = glob.glob(f'{data_root}*\images\*.png')
map_out_folder = 'maps_adaptive_kernel\\'
density_folder = 'D:\CNN\C-3-Framework\datasets\\NTU\processed_data\\train\\den'
...
for full_img_path in tqdm(img_pathes):
    ...
    results = np.append(results, [[img_path, round(abs(gt_points.shape[0] - np.sum(density_map)), 2)]], axis=0)
    
    curr_map_out_folder = data_folder + map_out_folder
    gt_out_path = curr_map_out_folder + img_path.strip('/').replace('.png', '.h5')
    density_path = density_folder + img_path.strip('/').replace('.png', '.csv')
    
    if not os.path.isdir(curr_map_out_folder):
        print('creating ' + curr_map_out_folder)
        os.makedirs(curr_map_out_folder)
    save_computed_density(density_map, gt_out_path, density_path)
    
    plt.imshow(img)
    plt.imshow(density_map, alpha=0.75)
    plt.savefig(gt_out_path.replace('.h5', '.png'))
    plt.show()
...
# for full_img_path in tqdm(img_pathes):
#     data_folder, img_path = full_img_path.split('images')
#     mat_path = full_img_path.replace('.png','.mat').replace('images','ground-truth').replace('IMG_','GT_IMG_')
